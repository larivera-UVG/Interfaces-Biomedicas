...
% EXTRACCIÓN CARACTERÍSTICAS DATOS DE ENTRENAMIENTO
load DATABASE_PROPIA3.mat

clase = 4;          %no. clases
m = 40;             %no. muestras x clase
muestras = clase*m; %no. muestras totales
n = 1000;           %no. puntos x canal
accuracy_rn = zeros(1,10);
accuracy_rn_t = zeros(1,10);


for i = 1:muestras
    v_mav(i,:) = [mav(data(i,1:n)),mav(data(i,n+1:end))];
    v_zc(i,:) = [zc(data(i,1:n),0),zc(data(i,n+1:end),0)];
    v_iemg(i,:) = [emg(data(i,1:n)),emg(data(i,n+1:end))];
    v_rms(i,:) = [rms(data(i,1:n)),rms(data(i,n+1:end))];
    v_wl(i,:) = [wl(data(i,1:n)),wl(data(i,n+1:end))];
    v_var(i,:) = [varianza(data(i,1:n)),varianza(data(i,n+1:end))];
    v_desv(i,:) = [desv(data(i,1:n)),desv(data(i,n+1:end))];
end
features = [v_mav,v_zc,v_wl,v_rms];   %Características

%EXTRACCIÓN DE CARACTERÍSTICAS DATOS DE PRUEBA
load 'datos_test_1.mat'                
muestras_t = 20;    %no. muestras

for i = 1:muestras_t
    v_mav_t(i,:) = [mav(data_t(i,1:n)),mav(data_t(i,n+1:end))];
    v_zc_t(i,:) = [zc(data_t(i,1:n),0),zc(data_t(i,n+1:end),0)];
    v_iemg_t(i,:) = [emg(data_t(i,1:n)),emg(data_t(i,n+1:end))];
    v_rms_t(i,:) = [rms(data_t(i,1:n)),rms(data_t(i,n+1:end))];
    v_wl_t(i,:) = [wl(data_t(i,1:n)),wl(data_t(i,n+1:end))];
    v_var_t(i,:) = [varianza(data_t(i,1:n)),varianza(data_t(i,n+1:end))];
    v_desv_t(i,:) = [desv(data_t(i,1:n)),desv(data_t(i,n+1:end))];
end
...
for i = 1:10
    ...
    clase_prueba = vec2ind(y_prueba);   %contiene las etiquetas predichas
    clase_real = vec2ind(labels_t);
    percentErrors_t = sum(clase_real ~= clase_prueba)/numel(clase_real);  
    accuracy_rn_t(1,i) = (1-percentErrors_t)*100;   %rendimiento
end
...
r = randn(muestras,10);   %Ordenas las muestras de entrenamiento en un orden random
...
r_t = randn(muestras_t,10);   %Ordenas las muestras de prueba en un orden random
...
for i = 1:10
    ...
    t_final_svm = toc(t_inicio_svm);

    accuracy_svm(:,i) = accuracy_L(1);   %rendimiento
end
A_SVM = mean(accuracy_svm);    %rendimiento promedio
